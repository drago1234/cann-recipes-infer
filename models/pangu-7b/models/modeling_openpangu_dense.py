...
# Overwrite the built-in print
builtins.print = global_print


def check(name, t):
    if torch.isnan(t).any() or torch.isinf(t).any():
        print(f"[NAN] {name}: nan={torch.isnan(t).any().item()} inf={torch.isinf(t).any().item()} "
              f"min={t.nan_to_num().min().item()} max={t.nan_to_num().max().item()} dtype={t.dtype} shape={tuple(t.shape)}")
...
class PanguEmbeddedDenseMLP(nn.Module):
    ...
    def forward(self, hidden_states, is_prefill=False, **kwargs):
        print(f"[PanguEmbeddedDenseMLP] Start running PanguEmbeddedDenseMLP.forward()! ")
    ...
    def moe_infer_double_routing(self, hidden_states, topk_ids, topk_weight):
        ...
        expanded_x, expanded_row_idx, tokens_per_expert, pertoken_scale = torch_npu.npu_moe_init_routing_v2(
            ...
            # scale=self.experts.smooth_scale_1 if self.gmm_quant_mode == "w8a8" and self.quant_type == QuantType.INT8 else None,1
...
class PanguEmbeddedModel(PanguEmbeddedPreTrainedModel):
    ...
    def __init__(self, config: PanguEmbeddedConfig, runner_settings, prefix: str = "", **kwargs):
        ...
        self.layers = nn.ModuleList(
            [
                PanguEmbeddedDecoderLayer(config, runner_settings, layer_idx, prefix=f"model.layers.{layer_idx}", **kwargs)
...
class PanguEmbeddedForCausalLM(PanguEmbeddedPreTrainedModel):
    ...
    def __init__(self, config, runner_settings, prefix: str = ""):
        ...
        # Initialize weights and apply final processing
        self.post_init()
        logger.info(f"check: input_max_len({runner_settings['data_config']['input_max_len']}) + max_new_tokens({runner_settings['data_config']['max_new_tokens']}) = {runner_settings['data_config']['input_max_len'] + runner_settings['data_config']['max_new_tokens']}")
            ...
            for (param_name, weight_name, shard_id) in stacked_params_mapping:
                ...
                for mapping in expert_params_mapping:
                    ...
                    if name.endswith(".bias") and name not in params_dict:
                        continue
                    if name not in params_dict:
                        print(f"[DEBUG] [MISSING WEIGHT]: '{name}' not found in params_dict={{list(params_dict.keys())[:10]}} Skipping...")
                        continue
                    param = params_dict[name]
                    weight_loader = getattr(param, "weight_loader",
                                            default_weight_loader)
